...
::: {style="font-size: 75%;"}
$$
\alpha_{\text{bayes}} \sim \mathcal{N}(0, s^2_\alpha) \qquad \bar{a} \mid \tau \sim \mathcal{N}(0, v_m) \qquad  v_m = \frac{\tau^2}{J}
$$
:::

Previously we saw that $\alpha_{\text{s2z}} = \alpha_{\text{bayes}} + \bar{a}$. Therefore, the joint prior given $\tau$ is MVN as

::: {style="font-size: 75%;"}
$$
\text{Var}(\alpha_{\text{s2z}} \mid \tau) = s^2_\alpha + v_m, \qquad \text{Cov}(\bar{a},\, \alpha_\text{s2z} \mid \tau) = v_m
$$
:::
where
...
::: {style="font-size: 75%;"}
$$
\begin{pmatrix} \bar{a} \\ \alpha_\text{s2z} \end{pmatrix}\Bigm|\tau
\sim
\mathcal N\!\left[
...
Using the standard conditional normal formulas, 

::: {style="font-size: 75%;"}
$$
\begin{gathered}
\bar{a}\mid\alpha_{\mathrm{s2z}},\,\tau
\sim\mathcal N\!\left(w\alpha_{\mathrm{s2z}},v_c\right),\\[0.6em]
...
## What about going from sum-to-zero to population models? {.s2z-conversion-code-slide}

Previously, we derived the distribution of $\bar{a}$, the realized mean of the Bayesian group effects, and we denote a random draw from that distribution as $m$. To convert the sum-to-zero estimates into equivalent (in distribution) Bayesian estimates:

1. Sample $m \sim \mathcal N(w\,\alpha_{\text{s2z}},v_c)$, using
   (mean, variance) notation.
2. Set $\alpha_{\text{bayes}} = \alpha_{\text{s2z}} - m$ and
   $a_j^{\text{bayes}} = a_j^{\text{s2z}} + m$.
...
The appendix code calls $z_{\bar b}$ `z_mean_b_cond`. Its symmetry means
$\lambda_\omega z_{\bar b}$ and $\omega z_{\bar b}$ have the same
distribution.
:::
