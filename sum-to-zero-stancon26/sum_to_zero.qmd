...
## Prior work and the missing connection {.nostretch .literature-positioning-slide}

::::: columns
::: {.column style="width: 59%; font-size: 57%; line-height: 1.12; border-right: 1px solid #d0d0d0; padding-right: 1.4em; box-sizing: border-box;"}
[Established pieces]{.emphasis2}

-   @gelman2005 distinguishes finite-population and superpopulation quantities.
-   @rouder2012 compare constrained fixed effects with exchangeable random effects.
-   @vines1996 introduce sweeping: replace $a_j$ by $a_j-\bar a$ and absorb $\bar a$ into the intercept.
-   @millar2009 show that sweeping changes inference from the population mean to the sample mean—and leave recovery as future work.
-   @ogle2020 extend sweeping to nested and crossed effect sets, but in the forward direction rather than from an S2Z fit back to the superpopulation posterior.
:::

::: {.column style="width: 41%; font-size: 60%; line-height: 1.14; padding-left: 1.4em; box-sizing: border-box;"}
[Missing connection]{.emphasis}
...
\qquad \bar a\mid\tau\sim\mathcal N(0, v_{\bar{a}}),\\[0.35em]
\alpha_{\mathrm{s2z}}=\alpha_{\mathrm{bayes}}+\bar a
\ \Longrightarrow\
\begin{cases}
\operatorname{Var}(\alpha_{\mathrm{s2z}}\mid\tau)=s_\alpha^2 + v_{\bar{a}}, \\
\operatorname{Cov}(\bar a,\alpha_{\mathrm{s2z}}\mid\tau) = v_{\bar{a}},
\end{cases}\\[0.45em]
\begin{pmatrix}\bar a \\ \alpha_{\mathrm{s2z}}\end{pmatrix}\Bigm|\tau
\sim\mathcal N\!\left[
\begin{pmatrix}0\\0\end{pmatrix},
\begin{pmatrix}v_{\bar{a}} & v_{\bar{a}} \\ v_{\bar{a}} &s_\alpha^2 + v_{\bar{a}} \end{pmatrix}
\right].
\end{gathered}
$$
...
\underbrace{\frac{v_{\bar{a}}}{s_\alpha^2 + v_{\bar{a}}}}_{w},
\qquad
\underbrace{v_{\bar{a}} - \frac{v_{\bar{a}}^2}{s_\alpha^2 + v_{\bar{a}}}}_{v_c}.
\end{gathered}
$$
:::
...
\alpha_{\mathrm{bayes}} = \alpha_{\mathrm{s2z}} - \bar{a}, \qquad
a_j^{\mathrm{bayes}} = a_j^{\mathrm{s2z}} + \bar{a},
$$
...
::: {.implications-pair .implications-math}
  $$
  \begin{aligned}
  \underbrace{\alpha_{\mathrm{s2z}}}_{\text{mean of varying }J\text{ groups}}
...
::: {.implications-pair .implications-math}
  ...
  \alpha_{\mathrm{Bayes}}^{\mathrm{rec}}
    &=\alpha_{\mathrm{s2z}} - \bar{a},\\
  \mathbf a_{\mathrm{Bayes}}^{\mathrm{rec}}
    &=\mathbf a_{\mathrm{s2z}}+ \bar{a} \mathbf 1.
  \end{aligned}
  $$
:::
...
## Appendix A · Posterior comparison diagnostics {.nostretch .appendix-detail-slide}

### On the difference between two posterior means {.appendix-subheading}
...
## Appendix B · Varying SDs · Conventional Bayesian implementation {.long-code-title .appendix-detail-slide}

### Differing variances conventional Bayesian Stan model: declarations {.appendix-subheading}
...
## Appendix B · Varying SDs · Conventional Bayesian implementation {.long-code-title .dense-code-slide .appendix-detail-slide}

### Differing variances conventional Bayesian Stan model: SD hierarchy and model {.appendix-subheading}
...
## Appendix C · Varying SDs · Sum-to-zero implementation {.appendix-detail-slide}

### S2Z Stan: declarations {.appendix-subheading}
...
### S2Z Stan: log-scale constraint {.appendix-subheading}
...
### S2Z Stan: remove the effect mean {.appendix-subheading}
...
### S2Z Stan: prior and likelihood {.appendix-subheading}
...
## Appendix D · Varying SDs · Reconstruction and marginalization {.nostretch .appendix-detail-slide}

### Completing the square in the removed mean $m$ {.appendix-subheading}
...
### Marginalizing the removed mean $m$ {.appendix-subheading}
...
## Appendix E · Elliptically symmetric distributions {.appendix-detail-slide}

### Distributions {.appendix-subheading}
