functions {
  ...
  matrix cholesky_corr_constrain_lp(int K, vector raw, int N_blocks,
    ...
    if (known_index[known_row, 1] == 2) {
      L[2, 1] = known_vals[1];
     // print("known_index[1, 1] = ", known_index[known_row, 1]);
      known_row += 1;
    } else {
    ...
    for (i in 3 : K) {
     ...
     // restricted case
      ...
      for (j in 2 : i - 1) {
        ...
        //  print("known_index[", known_row, ", 1] = ", known_index[known_row, 1], " and ", "i = ", i, " j = ", j);
        //  print("known_index[", known_row, ", 2] = ", known_index[known_row, 2], " and ", "i = ", i, " j = ", j);
    
          known_row = known_row == N_known ? N_known : known_row + 1;
        } else if (res_index[res_row, 1] == i && res_index[res_row, 2] == j) {
          if (res_id_cnt[res_id[res_row]] == 1) {
         //   if(i == 5 && j == 3) print("cnt = ", cnt, ", lb = ", lb[cnt], ", ub = ", ub[cnt]);
            L[i, j] = lb_ub_lp(raw[cnt], low, up);
            x_cache[res_id[res_row]] = L[i, j] * L[j, j] + b1;
            jac_cache[res_id[res_row]] = log(L[j, j]);
            res_id_cnt[res_id[res_row]] += 1;
            cnt += 1;
          //if (i == 5 && j == 3)   print("lb[cnt] = ", lb[cnt]);
          //print("cnt[", i, ",", j, "] = ", x_cache[res_id[res_row]],  " restricted");
          } else {
            L[i, j] = (x_cache[res_id[res_row]] - b1) / L[j, j];
           // target += log(abs(jac_cache[res_id[res_row]])); 
            target += -log(L[j, j]);
          }
